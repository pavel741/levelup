        ...
        if (firebaseUser && pathname?.startsWith('/auth')) {
          ...
          if (!userData) {
            ...
            try {
              ...
              // Store error
              try {
                const errorCode = error && typeof error === 'object' && 'code' in error && typeof error.code === 'string' ? error.code : 'firestore_create_failed'
                const errorMessage = error && typeof error === 'object' && 'message' in error && typeof error.message === 'string' ? error.message : 'Failed to create user in Firestore'
                localStorage.setItem('firebase_error', JSON.stringify({
                  code: errorCode,
